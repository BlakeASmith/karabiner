...
const modeTriggers = (mode: string, message?: string, ...manipulators: BasicManipulatorBuilder[]) => manipulators.map(
    (b) => {
        let it = b.toVar(mode)
        return message 
...
export function mode<ActionType>(mode: Mode<ActionType>): RuleBuilder[] {
    let triggersManipulators = modeTriggers(mode.name, mode.hint, ...mode.triggers) 
    let triggersRule = rule(`${mode.name}: ${mode.description}`).manipulators(triggersManipulators)
    return [
        triggersRule,
        rule(`Key assignments for ${mode.name}`)
            .manipulators(mode.manipulators.map(b => withModeExit(mode.name, b))),
        rule(`Escape ${mode.name}`)
            .manipulators([
